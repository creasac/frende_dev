-- Ensure sender upsert policy checks target membership through the secure
-- participant preferences function used by voice finalize.
...
begin
  ...
  return exists (
    select 1
    from public.get_conversation_participant_preferences_secure(conv_id) pref
    where pref.id = p_target_user_id
  );
end;
$function$;
