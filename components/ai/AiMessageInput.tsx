...
export default function AiMessageInput({
  ...
  }, [sessionId, disabled, isRecording, showTranscription])
  ...
  async function transcribeAudio(blob: Blob): Promise<string> {
    try {
      return await transcribeAudioWithRetry(blob, {
        language: 'auto',
        source: 'AiMessageInput',
        persist: true,
      })
    } catch (error) {
      console.error('[AiMessageInput] Transcription error:', error)
      return ''
    }
  }

  async function requestAiResponse(messageId: string) {
    try {
      await postJsonWithRetry(
        '/api/ai-chat',
        { sessionId, messageId },
        { source: 'AiMessageInput', persist: true }
      )
      onMessagesUpdated?.()
    } catch (err) {
      console.error('[AiMessageInput] Error:', err)
  ...
  async function sendMessageContent(content: string): Promise<boolean> {
    ...
    try {
      ...
      setSending(false)
      void requestAiResponse(data.id)
      return true
    } catch (err) {
      console.error('[AiMessageInput] Error:', err)
      setError(err instanceof Error ? err.message : 'Failed to send message')
  ...
  async function handleSend() {
    const wasSent = await sendMessageContent(text)
    if (wasSent) {
      setText('')
    }
  }
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          console.error('[AiMessageInput] Transcription failed:', error)
        } finally {
  ...
  async function sendTranscribedMessage() {
    const wasSent = await sendMessageContent(transcribedText)
    if (wasSent) {
      setTranscribedText('')
      setShowTranscription(false)
      setRecordingTime(0)
